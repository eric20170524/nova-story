...
export const CharacterManager: React.FC = () => {
  ...
  return (
    <div className="flex-1 bg-slate-950 p-8 overflow-y-auto h-full">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-bold text-white">{t('characters.title')}</h2>
        <button
          onClick={() => openModal()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {characters.map(char => (
          <div key={char.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-indigo-500/30 transition-all">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-lg text-slate-100">{char.name}</h3>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center sticky top-0 bg-slate-900 z-10">
              <h3 className="text-xl font-bold text-white">{editingChar.id ? t('characters.edit_title') : t('characters.new_title')}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div>
                 ...
                 <textarea 
                    className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white h-24"
              ...
              <div className="bg-slate-950 rounded-lg p-4 border border-slate-800">
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  <button type="button" onClick={handleAddTag} className="bg-slate-800 hover:bg-slate-700 px-3 rounded text-white"><Plus size={16} /></button>
                </div>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex justify-end pt-4">
                <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2 rounded-lg font-medium">
                  {t('characters.save')}
                </button>
              </div>
            </form>
