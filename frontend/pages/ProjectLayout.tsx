...
export const ProjectLayout: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="h-14 bg-slate-900 border-b border-slate-800 flex items-center px-6 gap-8 flex-shrink-0">
...
const TabLink: React.FC<{ to: string; icon: React.ReactNode; label: string }> = ({ to, icon, label }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
      `flex items-center gap-2 text-sm font-medium transition-colors border-b-2 py-4 ${
        isActive
