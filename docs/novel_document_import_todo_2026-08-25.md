...
选择 TXT / Markdown / NovaStory JSON
  ...
  → NovelImportDraft / NovaStoryJsonImportProject
  → 用户检查预览
...
POST /api/projects/import
  → commitProjectImportFile()
  → 与 canonical commit 相同的 parser / normalization / persistence strategy
```
...
- **统一 Native JSON 计划**：Preview 与 Restore 共用 `NovaStoryJsonImportProject`，不再分别解释原始 JSON。
- **Preview/Commit 一致**：预览计数必须等于实际可落库的 Chapter / Scene / Coverage 图，不显示原始但不可恢复的孤儿节点。
- **纯解析层**：Markdown / TXT / JSON 解析与 Native JSON normalization 不依赖数据库。
- **确定性导入**：只提取源文档明确提供的信息，不用 AI 猜人物或世界观。
- **信息不丢失**：无法映射的小节进入 `settings.import_info.unmapped_sections`；重复正文/概要会合并而不是丢弃。
...
- [x] 中文 / 英文章标题识别无回归
- [x] 无结构 TXT 降级为单章
- [x] `.txt` 即使以 `{` 开头，也只有在确实是可识别项目 JSON 时才切换到 Native JSON 路径
...
- [x] 结构 heading 不进入正文
- [x] 章节正文前置段落不丢失
- [x] 标题与首个 `##` 之间的项目前言保留为 unmapped project section
- [x] 重复 `### 章节概要` 按顺序合并
- [x] 重复 `### 正文` 按顺序合并
- [x] 重复创作信息 section 合并 tags / import metadata
- [x] 章节之后的非章节 `##` 资料不丢失
...
- [x] 非项目 JSON object → 400
- [x] parser 层无数据库依赖

## Native JSON canonical plan

- [x] 新增纯数据模型 `NovaStoryJsonImportProject`
- [x] project title / description / settings 统一规范化
- [x] chapter / character / scene / coverage graph 统一规范化
- [x] Preview 与 Restore 消费同一份 normalized plan
- [x] Preview 数量只统计实际可恢复节点
- [x] Scene 引用不存在的 Chapter → warning + 从计划排除
- [x] CoverageGroup 引用不存在的 Scene → warning + 从计划排除
- [x] CoverageShot 引用不存在的 Group → warning + 从计划排除
- [x] 重复 Chapter / Scene / CoverageGroup source ID → 400，避免歧义映射
- [x] settings 非 object / malformed JSON → `{}` + warning，不在 persistence 阶段 500
- [x] title / description / body 类型异常不会直接传入 SQLite driver
...
- [x] 失败 rollback
- [x] Restore 不再重新解释 raw JSON，只执行 normalized plan
...
- [x] canonical 与 legacy 共享同一 parser / normalization / persistence strategy
...
- [x] project preamble preservation
- [x] repeated summary/content merge
- [x] repeated creation metadata merge
- [x] single-chapter fallback
- [x] TXT 既有测试无回归

## Preview / Commit / Native JSON tests
...
- [x] Native JSON orphan graph normalization warnings
- [x] Native JSON duplicate ID → 400 before DB
- [x] arbitrary JSON object → 400
- [x] Native Preview counts === committed DB graph counts
...
- [x] 返回章节 / 概要 / 正文 / 人物数量
- [x] 返回实际可恢复的 JSON scene / coverage / shot 数量
- [x] 返回 normalization / parser warnings
- [x] 返回 unmappedSections
- [x] summary preview 截断，避免无必要回传完整长概要
...
Legacy /projects/import
  → commitProjectImportFile()
  → 与 canonical commit 完全相同的解析、规范化和持久化链路
```
