...
echo [0/3] 正在检查并清理已占用的旧进程 (端口 8188, 3000, 3000)...
powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort 8188,3000,3000 -State Listen -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id `$_.OwningProcess -Force -ErrorAction SilentlyContinue }" >nul 2>&1
for /f "tokens=5" %%a in ('netstat -ano ^| findstr /C:":8188 " /C:":3000 " /C:":3000 " ^| findstr LISTENING') do (
...
echo [2/3] 正在拉起 NovaStory 后端 FastAPI 服务 (端口 3000)...
start "NovaStory Backend (3000)" cmd /k "chcp 65001 >nul && cd /d %~dp0backend && .\\.venv\\Scripts\\python.exe main.py"
...
echo  - 后端 API 服务:     http://localhost:3000/api/docs
...
echo ========================================================
echo.
echo 正在停止占用端口 8188, 3000, 3000 的服务进程...
powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort 8188,3000,3000 -State Listen -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id `$_.OwningProcess -Force -ErrorAction SilentlyContinue }" >nul 2>&1
for /f "tokens=5" %%a in ('netstat -ano ^| findstr /C:":8188 " /C:":3000 " /C:":3000 " ^| findstr LISTENING') do (
    taskkill /F /PID %%a >nul 2>&1
)
