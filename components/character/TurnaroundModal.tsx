...
export const TurnaroundModal: React.FC<TurnaroundModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
      <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl custom-scrollbar animate-in zoom-in-95 duration-200">
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 flex flex-col justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-950 border border-indigo-200 dark:border-indigo-700/50 text-indigo-700 dark:text-indigo-200 text-xs font-semibold">
                  {modelType === 'sd15' ? 'SD 1.5 Draft' : modelType === 'redcraft_krea2' ? 'RedCraft 3.0 (Krea2)' : 'Pony XL (SDXL)'}
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium">
                  画风: {projectStyle}
                </span>
                <span
              ...
              <div className="flex gap-2.5">
                <button
                  ...
                  className={`flex-1 py-2 px-3 rounded-xl text-xs font-semibold border transition-all ${
                    genType === 'turnaround'
                      ? 'bg-indigo-50 dark:bg-indigo-600/30 border-indigo-300 dark:border-indigo-500 text-indigo-700 dark:text-indigo-200 shadow-xs'
            ...
            <div className="flex items-center justify-between">
              ...
              {refImageUrl && (
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-xs text-slate-700 dark:text-slate-300 font-semibold">{t('characters.use_ref_portrait')}</span>
                </label>
              )}
            </div>

            {refImageUrl ? (
              <div className="flex items-center gap-4 bg-white dark:bg-slate-900/80 p-3.5 rounded-xl border border-slate-200/80 dark:border-slate-800">
                <PreviewableImage
                  src={refImageUrl}
                  alt="Reference Portrait"
                  className="w-16 h-16 rounded-xl object-cover border border-emerald-300 dark:border-emerald-500/50 shadow-xs"
                />
              ...
              <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/40 rounded-xl text-xs text-amber-800 dark:text-amber-300 flex items-center justify-between">
                <span>{t('characters.no_ref_portrait')}</span>
                <label className="cursor-pointer bg-amber-100 hover:bg-amber-200 dark:bg-amber-900/40 dark:hover:bg-amber-800/60 text-amber-800 dark:text-amber-200 px-2.5 py-1 rounded-lg text-[11px] font-semibold border border-amber-300 dark:border-amber-700/50 flex items-center gap-1 transition-all shadow-2xs">
          ...
          {genType === 'turnaround' && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 mr-1">{t('characters.quick_tags_label')}:</span>
              <button
                type="button"
                onClick={() => setPrompt((p) => (p ? `${p}, front view, side view, back view` : 'front view, side view, back view'))}
                className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/80 dark:hover:bg-indigo-900 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-700/50 rounded-lg text-[11px] font-mono transition-all font-medium"
              >
              ...
              <button
                ...
                className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-lg text-[11px] font-mono transition-all font-medium"
              >
                {t("casting.white_bg")}
              </button>
              <button
                ...
                className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/80 dark:hover:bg-emerald-900 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700/50 rounded-lg text-[11px] font-mono transition-all font-medium"
              >
          ...
          <div>
            ...
            <textarea
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-slate-900 dark:text-white text-xs sm:text-sm font-mono h-24 focus:ring-2 focus:ring-indigo-500/50 focus:outline-none transition-all"
              value={prompt}
          ...
          {/* Negative Prompt Box */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">{t('characters.negative_prompt')}</label>
            <input
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-slate-800 dark:text-slate-300 text-xs font-mono focus:ring-2 focus:ring-indigo-500/50 focus:outline-none transition-all"
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-4 flex flex-col items-center">
            <button
              ...
              className="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 disabled:opacity-50 text-white font-semibold px-8 py-3 rounded-xl flex items-center justify-center gap-2 shadow-md shadow-indigo-500/20 transition-all"
            >
              {isPromptLoading ? (
                <>
            ...
            {generatedImageUrl && (
              <div className="mt-6 w-full flex flex-col items-center space-y-4">
                <div className="relative rounded-2xl overflow-hidden border border-indigo-300 dark:border-indigo-500/40 bg-slate-900 max-h-80 shadow-xl group">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  <button
                    ...
                    className="bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/60 dark:hover:bg-emerald-600 text-emerald-800 dark:text-emerald-100 dark:hover:text-white px-3 py-2 rounded-xl text-xs font-semibold border border-emerald-300 dark:border-emerald-500/40 flex items-center gap-1.5 transition-all shadow-2xs"
                    title="覆盖当前版本立绘"
                  >
                  ...
                  <button
                    ...
                    className="bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/50 dark:hover:bg-amber-700 text-amber-800 dark:text-amber-100 dark:hover:text-white px-3 py-2 rounded-xl text-xs font-semibold border border-amber-300 dark:border-amber-500/40 flex items-center gap-1.5 transition-all shadow-2xs"
                    title="新建版本并保存为立绘"
                  >
                  ...
                  <button
                    ...
                    className="bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/60 dark:hover:bg-indigo-600 text-indigo-800 dark:text-indigo-100 dark:hover:text-white px-3 py-2 rounded-xl text-xs font-semibold border border-indigo-300 dark:border-indigo-500/40 flex items-center gap-1.5 transition-all shadow-2xs"
                    title="覆盖当前版本三视图"
                  >
