...
export const SceneVideoPlayer: React.FC<SceneVideoPlayerProps> = ({
  ...
  const renderUploadButton = (
    ...
    <label
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[11px] font-medium cursor-pointer transition-all shadow-sm ${
        uploadingRole === role
          ? 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-400 dark:text-slate-500 cursor-wait'
          : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-300 hover:shadow'
      }`}
    >
  ...
  const renderReferenceManager = () => {
    ...
    return (
      <div className="p-3.5 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-indigo-900/50 space-y-3.5 text-[11px] transition-colors">
        ...
        {suggestedWorkflow && (
          <div className="p-2.5 rounded-xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-800/60 flex items-start justify-between gap-2.5 shadow-sm">
            <div className="text-[11px] text-indigo-900 dark:text-indigo-200 leading-relaxed">
              <span className="font-semibold text-indigo-700 dark:text-indigo-300">当前镜头素材建议：</span>
        ...
        <div className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">Workflow Strategy</span>
              ...
              <button
                ...
                className={`p-2.5 rounded-xl border text-left transition-all ${
                  workflowId === workflow.id
                    ...
                    : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-semibold text-xs">{workflow.label}</span>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-medium border ${
                    workflow.id === 'minimax_h3_hongchao_a2a_12gb'
                      ? 'text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40'
                      : workflow.id === 'minimax_h3_multiframe_official_12gb'
                        ...
                        : 'text-sky-700 dark:text-sky-300 border-sky-300 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/40'
                  }`}>
                    {workflow.badge}
                  </span>
                </div>
                <p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">{workflow.description}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="rounded-xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 p-2.5 space-y-1.5 shadow-sm">
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-slate-800 dark:text-slate-300 flex items-center gap-1.5 text-xs">
                <ImageIcon size={13} className="text-indigo-600 dark:text-indigo-400" />
                First Frame <span className="text-rose-500">*</span>
              </span>
          ...
          {isMultiframe && (
            <div className="rounded-xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 p-2.5 space-y-2 shadow-sm">
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold text-slate-800 dark:text-slate-300 flex items-center gap-1.5 text-xs">
          ...
          <div className={`rounded-xl bg-white dark:bg-slate-900/70 border p-2.5 space-y-1.5 shadow-sm ${isRef2va ? 'border-slate-200 dark:border-slate-800 opacity-55' : 'border-slate-200 dark:border-slate-800'}`}>
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-slate-800 dark:text-slate-300 flex items-center gap-1.5 text-xs">
                <Link2 size={13} className="text-emerald-600 dark:text-emerald-400" />
                Last Frame
              </span>
              ...
              <>
                ...
                {selectedKeyframeId && (
                  <button
                    ...
                    className="text-[10px] text-indigo-600 dark:text-indigo-300 hover:text-indigo-700 dark:hover:text-indigo-200 underline underline-offset-2"
                  >
          ...
          <div className={`rounded-xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 p-2.5 space-y-1.5 shadow-sm ${isFl2va ? 'opacity-55' : ''}`}>
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-slate-800 dark:text-slate-300 flex items-center gap-1.5 text-xs">
            ...
            {isFl2va ? (
              <div className="text-[10px] text-amber-600 dark:text-amber-400">FL2VA 不消费人物 identity reference。</div>
            ) : characterReferenceAssets.length === 0 ? (
              <div className="text-[10px] text-slate-400 dark:text-slate-600">暂无人物参考图</div>
            ) : (
              <div className="space-y-1 max-h-28 overflow-y-auto custom-scrollbar">
                {characterReferenceAssets.map((asset) => (
                  <label key={asset.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/70 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
          ...
          <div className={`rounded-xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 p-2.5 space-y-1.5 shadow-sm ${isFl2va ? 'opacity-55' : ''}`}>
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-slate-800 dark:text-slate-300 flex items-center gap-1.5 text-xs">
                <Film size={13} className="text-sky-600 dark:text-sky-400" />
                Motion Ref
              </span>
              {!isFl2va && renderUploadButton('motion_reference', '上传', 'video/*', <Upload size={10} />)}
            </div>
            {isFl2va ? (
              <div className="text-[10px] text-amber-600 dark:text-amber-400">FL2VA 不消费 motion reference。</div>
            ) : (
              <select
  ...
  if (isGenerating) {
    ...
    return (
      <div className="w-full h-full min-h-[220px] bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 text-center select-none relative overflow-hidden transition-colors">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 dark:from-indigo-950/30 to-purple-500/10 dark:to-purple-950/30 animate-pulse pointer-events-none" />
        <Loader2 className="animate-spin text-indigo-600 dark:text-indigo-400 mb-3" size={32} />
        <span className="text-xs font-semibold text-indigo-900 dark:text-indigo-200 mb-1">
          {t('director.generating_video_status', 'H3 视频生成中...')}
        </span>
        <span className="text-[11px] font-mono px-3 py-1 rounded-lg bg-indigo-100/80 dark:bg-indigo-950/80 border border-indigo-200 dark:border-indigo-700/50 text-indigo-800 dark:text-indigo-300 mb-1">
          {stageLabels[stage] || stage}
        </span>
        {typeof taskState?.queue_position === 'number' && taskState.queue_position > 0 && (
          <span className="text-[10px] text-slate-500 dark:text-slate-400 mb-3">GPU Queue #{taskState.queue_position}</span>
        )}
        {taskState?.task_id && onCancelTask && (
          <button
            type="button"
            onClick={() => onCancelTask(taskState.task_id)}
            className="px-3.5 py-1.5 rounded-lg text-xs bg-white dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950 text-slate-600 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-300 border border-slate-200 hover:border-rose-300 dark:border-slate-700 dark:hover:border-rose-800 transition-all shadow-sm"
          >
            取消任务
          </button>
        )}
  ...
  if (!videoUrl) {
    return (
      <div className="w-full bg-slate-50 dark:bg-slate-950/90 flex flex-col select-none border-b border-slate-200 dark:border-slate-800 transition-colors">
        <div className="min-h-[220px] flex flex-col items-center justify-center p-4 text-center">
          <div className="w-12 h-12 rounded-2xl bg-indigo-100 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800/40 flex items-center justify-center mb-3 text-indigo-600 dark:text-indigo-400 shadow-sm">
            <Video size={22} />
          </div>
          <p className="text-xs text-slate-800 dark:text-slate-300 font-semibold mb-1">
            {t('director.no_video_yet', '暂未生成 H3 视频镜头')}
          </p>
          <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-3 max-w-[240px]">
          ...
          {onGenerateVideo && (
            <button
              ...
              className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-xs font-semibold px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-md shadow-indigo-600/20 hover:shadow-indigo-500/30 transition-all"
            >
  ...
  return (
    <div className="w-full flex flex-col bg-black relative group/player overflow-hidden select-none">
      ...
      <div className="p-2.5 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between gap-1.5 text-xs transition-colors">
        {canPromote && onPromoteAsset && activeAsset && (
          <button
            ...
            className="flex-1 bg-indigo-50 dark:bg-indigo-600/30 hover:bg-indigo-100 dark:hover:bg-indigo-600/50 border border-indigo-200 dark:border-indigo-500/50 text-indigo-700 dark:text-indigo-200 py-1.5 px-2.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-sm"
        ...
        {canReprocess && onReprocessAsset && activeAsset && (
          <button
            ...
            className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 py-1.5 px-2.5 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 border border-slate-200 dark:border-slate-700 transition-colors shadow-sm"
        ...
        {onGenerateVideo && (
          <button
            ...
            className={`py-1.5 px-2.5 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 border transition-all shadow-sm ${
              showReferenceManager
                ? 'bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-200 border-indigo-300 dark:border-indigo-700'
                : 'bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950 hover:text-indigo-700 dark:hover:text-indigo-200 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700'
            }`}
      ...
      {showQADetails && qaReport && (
        <div className="p-3 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 space-y-2.5 animate-in slide-in-from-top-2 duration-150 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">连续性总分 (0-100):</span>
              <span className="font-bold text-emerald-600 dark:text-emerald-400 text-xs">
                {(qaReport.continuity_scores?.normalized_score ?? 0).toFixed(0)}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">接缝成本:</span>
              <span className="text-rose-600 dark:text-rose-300">
                {(qaReport.continuity_scores?.seam_cost ?? 0).toFixed(2)}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">外观误差:</span>
              <span className="text-indigo-600 dark:text-indigo-300">
                {(qaReport.continuity_scores?.appearance_error ?? 0).toFixed(2)}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">运动边界误差:</span>
              <span className="text-amber-600 dark:text-amber-300">
                {(qaReport.continuity_scores?.motion_error ?? 0).toFixed(2)}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">亮度闪烁:</span>
              <span className="text-sky-600 dark:text-sky-300">
                {(qaReport.continuity_scores?.flicker_error ?? 0).toFixed(2)}
              </span>
            </div>
            <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-500 dark:text-slate-400 block mb-0.5">质量门:</span>
              <span className={`font-semibold ${qaReport.quality_grade === 'pass' ? 'text-emerald-600 dark:text-emerald-300' : qaReport.quality_grade === 'manual_review' ? 'text-amber-600 dark:text-amber-300' : 'text-rose-600 dark:text-rose-300'}`}>
          ...
          {qaReport.reasons && qaReport.reasons.length > 0 && (
            <div className="text-[10px] text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-950 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="text-slate-700 dark:text-slate-300 font-semibold block mb-1">评估备注:</span>
